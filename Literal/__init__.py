...
def reprargs(args, sep=', ', filter=repr):
    return sep.join((list(map(filter, args))))
...
def reprsig(fun, name=None, method=False):
    ...
    if defs:
        ...
        kwargs = dict(list(zip(kwkeys, defs)))
    if varargs:
        args.append('*' + varargs)
    if method:
        args.insert(0, 'self')
    return reprcall(name, list(map(str, args)), kwargs, keywords,
                    argfilter=str)
...
class node(_node, metaclass=_ops):
    ...
    def infix(self, op, rhs):
        n = self._new_node(
                ' '.join(map(str, [self.value, op, get_value(rhs)])))
        self.remove(self)
    ...
    def __call__(self, *args, **kwargs):
        ...
        list(map(self.remove, _args + _values))
        return n
...
class root(_node):
    ...
    def __unicode__(self):
        return '\n'.join([_f for _f in map(str, self) if _f])

    def __iter__(self):
        for node in self.__next__:
            yield node
